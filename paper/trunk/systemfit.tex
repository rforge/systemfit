...
\Abstract{ 
  ...
  structurally related equations. If the cross-equation disturbances
  are correlated, full information methods are required
  \citep{zellner62}. If exogenous variables are stochastically
  dependent on the disturbances in the system
  \citep{zellner62b,theil71}, limited information methods should be
  used for parameter estimation. The package \pkg{systemfit} provides
  the capability to estimate systems of linear and non-linear
  structural equations for limited (ordinary least squares (OLS), two
  stage-least squares (2SLS)) and full-information
  (seemingly-unrelated regression (SUR) and three-stage least squares
  (3SLS)) methods within the \proglang{R} programming environment.}
  
\Keywords{R, simultaneous equations estimation, ordinary least
  squares, seemingly unrelated regression, two-stage least squares,
