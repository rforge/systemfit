...
\Abstract{Many statistical analysis are based on sets of models that
  consist of  more than one equation which contain relationships among
  the various equations.  Ignoring these relationships can result in
  biased and possibly inconsistent parameter estimates. These sets of
  equations should be fitted simultaneously rather than equation  by
  equation.  The package \pkg{systemfit} provides the capability to
  estimate systems of linear and non-linear equations, using limited
  and full-information methods within the \proglang{R} programming
  environment.  }
