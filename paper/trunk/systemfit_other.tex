...
($\SHat = \left[ \sHat_{ij} \right]$)
can be calculated from the residuals of a first-step OLS or 2SLS estimation.
In many cases, the formula
\begin{equation}
   \sHat_{ij} = \frac{ \widehat{u}_i' \widehat{u}_j }{ T }
   \label{eq:rcovNoDfCor}
\end{equation}
is applied, where $T$ is the number of observations in each equation.
...
matrix are
\begin{equation}
   \sHat_{ij} = \frac{ \widehat{u}_i' \widehat{u}_j }
...
If equation (\ref{eq:rcovNoDfCor}) is applied to calculate the estimated
...
for $\widehat{u}_i$ in (\ref{eq:rcovNoDfCor}--\ref{eq:rcovTheil}).
...
\begin{equation}
   R_*^2 = 1 - \frac{ \widehat{u}' \OHat^{-1} \widehat{u} }
...
F = \frac{
   ...
   \widehat{u}' ( \SHat \otimes I )^{-1} \widehat{u} /
   ( G \cdot T - K )
} \; ,
\end{equation}
...
\begin{equation}
  m = \left( \bHat_{2SLS} - \bHat_{3SLS} \right)^{'}
      \left( \COV \left[ \bHat_{2SLS} \right] -
             \COV \left[ \bHat_{3SLS} \right] \right)
      \left( \bHat_{2SLS} - \bHat_{3SLS} \right) ,
\label{eq:hausman}
\end{equation}
where $\bHat_{2SLS}$ and $\COV \left[ \bHat_{2SLS} \right]$ are the estimated
coefficient and covariance matrix from 2SLS estimation, and
$\bHat_{3SLS}$ and $\COV \left[ \bHat_{3SLS} \right]$ are the estimated
