...
The software LIMDEP calculates SUR or 3SLS estimates by a
slightly different approach.
It uses the residuals of a first-step OLS or 2SLS estimation
for a second-step WLS or W2SLS estimation,
and then calculates the residual variance covariance matrix
from the residuals of the WLS or W2SLS estimation
...
residual variance-covariance matrix, the results converge to maximum
...
the variance covariance matrix of the parameters is calculated
by~(\ref{eq:olsVarSingleSigma}),
...
the variance covariance matrix of an OLS estimation is calculated
by~(\ref{eq:olsVarSameSigma}),
...
The resulting test statistic, distributed as a
$\chi^2$, is the difference between the covariance of an
3SLS estimator and the covariance of an 2SLS estimator.
If the difference is (close to) zero, the model is correctly specified.
The Hausman test statistic is,
\begin{equation}
  m = \left( \hat{\beta}_2 - \hat{\beta}_3 \right)^{'}
      \left( Var \left[ \hat{\beta}_2 \right] -
             Var \left[ \hat{\beta}_3 \right] \right)
      \left( \hat{\beta}_2 - \hat{\beta}_3 \right)
\label{eq:hausman}
\end{equation}
where $\hat{\beta}_2$ and $Var \left[ \hat{\beta}_2 \right]$ are the estimated
coefficient and variance-covariance matrix from 2SLS estimation, and
$\hat{\beta}_3$ and $Var \left[ \hat{\beta}_3 \right]$ are the estimated
coefficients and variance covariance-matrix from 3SLS estimation.
