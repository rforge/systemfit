...
alternatives for which $\hat{\beta}_2$ and $\hat{\beta}_3$ diverge
under misspecification.
The resulting test statistic, distributed as a
$\chi^2$, is the difference between the covariance of an
efficient estimator and the covariance of the inefficient estimator.
If the difference is zero, the model is correctly specified.
...
Under the alternative hypothesis the 2SLS estimator is consistent but the
3SLS estimator is inconsistent.

The Hausman test statistic is,

\begin{equation}
  m = ( \hat{\beta}_2 - \hat{\beta}_3 )^{'} ( \hat{\Omega}_2 - \hat{\Omega}_3 ) ( \hat{\beta}_2 - \hat{\beta}_3 ) 
\label{eq:hausman}
\end{equation}
where $\hat{\beta}_2$ and $\hat{\Omega}_2$ are the estimated
coefficient and variance-covariance matrix from 2SLS estimation and
$\hat{\beta}_3$ and $\hat{\Omega}_3$ are the estimated coefficients
and variance covariance-matrix from 3SLS estimation.
