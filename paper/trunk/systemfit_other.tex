...
\begin{equation}
   ...
   \left( I_T - \frac{\iota \iota'}{T} \right) \right) y } ,
\end{equation}
...
to replace the unknown $\Sigma$ in~(\ref{eq:f-test})
...
because the numerator of~(\ref{eq:f-test-theil})
converges in probability to the numerator of~(\ref{eq:f-test})
and the denominator of~(\ref{eq:f-test-theil})
converges in probability to the denominator of~(\ref{eq:f-test})
\citep[p.~402]{theil71}.

Furthermore, the denominators of both~(\ref{eq:f-test})
...
($\E \left[ u_i' Z_j \right] = 0 \, \forall \, i \neq j$).
