...
($\E \left[ u_i' Z_i \right] = 0$).

At the first stage new ('fitted') regressors are obtained by
\begin{equation}
   \XHat_i = Z_i \left( Z_i' Z_i \right)^{-1} Z_i' X_i .
\end{equation}
...
$( \E \left[ u_i' Z_j \right]) = 0 \; \forall \; i, j )$.
...
\begin{equation}
   \bHat_{3SLS-GMM} = \left( X' Z \left( Z' \OHat Z \right)^{-1}
   Z' X \right)^{-1} X' Z \left( Z' \OHat Z \right)^{-1} Z' y
   \label{eq:3slsGmm}
\end{equation}
with
\begin{equation}
...
\begin{align}
   ...
   \left( \XHat' \OHat^{-1}  \XHat \right)^{-1}
   \XHat' \OHat^{-1} Z \left( Z' Z \right)^{-1} Z' \OHat Z \\
   & \left( Z' Z \right)^{-1} Z' \OHat^{-1} \XHat
   \left( \XHat' \OHat^{-1}  \XHat \right)^{-1} .
   \nonumber
\end{align}
