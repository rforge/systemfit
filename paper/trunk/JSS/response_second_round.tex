...
We have clarified our notations by avoiding the ambiguous word °impose°.
Instead, we now clearly describe whether we mean
°estimating coefficients under restrictions°
or °testing restrictions on the coefficients°.

\subsubsection{Methods for classes ``systemfit'' and ``systemfit.equation''}
...
We have reorganized section~3 (°Source code°).
As suggested by this referee,
we now start this section with a description
of the core function \code{systemfit()}
and introduce the classes \code{systemfit} and \code{systemfit.equation}.
Thereafter, we explain the methods
that are available for these classes.
However, we did not follow the suggestion of this referee
to introduce the usage and the most important arguments of \code{systemfit()} here,
because the knowledge about these arguments is not required
to understand this section
and --- in our opinion --- it is more logical to do this
in the following section (°Using systemfit°).
...
We now refer to the methods in the \pkg{systemfit} package
by the names of these methods
(and no longer by the full names of these functions).
...
The first argument of systemfit() has been renamed to \code{formula}.
...
We have clarified the term °panel-like data°
by renaming it as °panel data°.

%     Also Section 4.3 interrupts the flow of Section 4.2 and 4.3
%     and should be postponed.
Section~4.3 (°Panel data°, now section~4.4) has been postponed
to not interrupt the flow of
sections~4.2 (°User options of \code{systemfit}°)
and~4.4 (°Summary results with \code{summary}\hspace{0pt}\code{.systemfit}°,
now section~4.3).
...
%     (which are also much shorter to type).
The arguments \code{returnModelFrame}, \code{returnModelMatrix},
\code{returnResponse}, and \code{returnInst}\hspace{0pt}\code{Matrix}
of \code{systemfit()}
have been renamed as \code{model}, \code{x}, \code{y}, and \code{z},
respectively, to be consistent with standard regression tools in
\proglang{R}, such as \code{lm()}.
Analogously, the elements \code{modelFrame}, \code{modelMatrix},
\code{response}, and \code{instMatrix}
returned by \code{systemfit()}
have been renamed as \code{model}, \code{x}, \code{y}, and \code{z},
respectively.
% revision 506--513 of the systemfit package

%     Similarly, "printResidCov"
%     and "printEquations" are extremely long.
Furthermore, the arguments \code{printResidCov} and \code{printEquations}
of \code{summary}\hspace{0pt}\code{.systemfit()}
...
We have made the description of the $F$ test and Wald test(s)
clearer and more elaborate now (see Section~2.7).
Furthermore, we have extended the method \code{linear.hypothesis}
for objects of class \code{systemfit}
so that it can calculate all three test statistics now
(see Section~4.5).
...
%         freedm than summary().
Statistical tests on the coefficients can use either
the degrees of freedom of a single equation
or the degrees of freedom of the entire system.
If \code{df.residual} is applied to an object of class \code{systemfit},
it returns the degrees of freedom of the entire system.
Similarly,
if \code{linear.hypothesis} is applied to an object of class \code{systemfit},
it uses the degrees of freedom of the entire system.
The argument \code{useDfSys} of \code{summary.systemfit} determines,
whether the degrees of freedom of a single equation
or the degrees of freedom of the entire system
are used to calculate the $P$ values for the $t$~tests on the coefficients.
Section~4.3.1 (°Degrees of freedom for $t$ tests°) of the paper
explains how the default value for argument \code{useDfSys} is determined.
In the example provided by this referee,
the \code{summary} method uses the degrees of freedom of each single equation
(unlike \code{linear.hypothesis}).
One can obtain the same results as with \code{linear.hypothesis}
by adding the argument \code{useDfSys = TRUE} to the \code{summary} call.
...
We have modified the code for obtaining the object names in
the \code{linear.hypothesis} and \code{lrtest} methods
according to the suggestion of this referee.
...
%     estimator.
The coefficients of the model $(\beta)$ are consistently called
°coefficients° now
(and no longer °parameters° every now and then).
...
In this point, we disagree with the referee
and claim that the formulas in our paper are correct:
$y$ should \emph{not} be replaced by $\hat{y}$.
You can confirm this by taking a look, for instance,
at equation (15-21) in \citet[p.~399]{greene03}
(please note that \citeauthor{greene03}'s $Z$ corresponds to our $X$
and \citeauthor{greene03}'s $X$ corresponds to our $H$).
